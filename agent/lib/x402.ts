/**
 * x402 payment protocol — lets the automaton pay for paid HTTP APIs with USDC
 * micropayments, signed by its own auto-generated EVM wallet (agent/lib/wallet.ts).
 * Ported from the original repo's src/conway/x402.ts, trimmed to Base/Base
 * Sepolia + EVM wallets (this rebuild's ERC-8004 work is Base-only already).
 *
 * Protocol: request a paid URL; a 402 response carries payment requirements
 * (amount, recipient, USDC contract); sign an EIP-3009 TransferWithAuthorization
 * for that amount and retry with the signature in an X-Payment header.
 */
...
function normalizeNetwork(raw: unknown): NetworkId | null {
  ...
  if (normalized === "eip155:8453" || normalized === "eip155:84532") return normalized;
  return null;
}
...
function normalizePaymentRequirement(raw: unknown): PaymentRequirement | null {
  ...
  const requiredDeadlineSeconds =
    parsePositiveInt(value.requiredDeadlineSeconds) ?? parsePositiveInt(value.maxTimeoutSeconds) ?? 300;

  if (!scheme || !maxAmountRequired || !payToAddress || !usdcAddress) return null;
...
function normalizePaymentRequired(raw: unknown): PaymentRequiredResponse | null {
  ...
  return { x402Version: parsePositiveInt(value.x402Version) ?? 1, accepts };
}
...
function parseMaxAmountRequired(maxAmountRequired: string, x402Version: number): bigint {
  ...
  if (amount.includes(".")) return parseUnits(amount, 6);
  if (x402Version >= 2 || amount.length > 6) return BigInt(amount);
...
function selectRequirement(parsed: PaymentRequiredResponse): PaymentRequirement {
  ...
  return exactSupported ?? parsed.accepts[0];
}

async function parsePaymentRequired(resp: Response): Promise<ParsedPaymentRequirement | null> {
  const header = resp.headers.get("X-Payment-Required");
  if (header) {
    const normalizedRaw = normalizePaymentRequired(safeJsonParse(header));
    if (normalizedRaw) {
      return { x402Version: normalizedRaw.x402Version, requirement: selectRequirement(normalizedRaw) };
    }
    try {
      const decoded = Buffer.from(header, "base64").toString("utf-8");
      const parsedDecoded = normalizePaymentRequired(safeJsonParse(decoded));
      if (parsedDecoded) {
        return { x402Version: parsedDecoded.x402Version, requirement: selectRequirement(parsedDecoded) };
      }
    } catch {
  ...
  try {
    ...
    return { x402Version: parsedBody.x402Version, requirement: selectRequirement(parsedBody) };
  } catch {
    return null;
  }
}
...
async function signPayment(
  ...
  if (!chain) throw new Error(`Unsupported network: ${requirement.network}`);
...
/** Get the automaton's USDC balance on Base (or Base Sepolia). EVM wallets only. */
export async function getUsdcBalanceDetailed(
  ...
  if (!chain || !usdcAddress) {
    return { balance: 0, network, ok: false, error: `Unsupported USDC network: ${network}` };
  }
  try {
    const client = createPublicClient({ chain, transport: http(process.env.RPC_URL, { timeout: 10_000 }) });
...
/**
 * Fetch a URL with automatic x402 USDC payment. If the server responds 402,
 * signs a TransferWithAuthorization for the requested amount (capped at
 * maxPaymentCents) and retries with the signature attached.
 */
export async function x402Fetch(
  ...
  if (chainType === "solana") {
    return { success: false, error: "x402 payment requires an EVM wallet. This automaton's wallet is Solana." };
  }
  ...
  try {
    ...
    if (!parsed) {
      return { success: false, error: "Could not parse payment requirements", status: initialResp.status };
    }
    ...
    if (maxPaymentCents !== undefined) {
      ...
      const amountCents = Number(amountAtomic) / 10_000; // atomic (6dp) -> cents (2dp)
    ...
    try {
      ...
      return {
        success: false,
        error: `Failed to sign payment: ${err instanceof Error ? err.message : String(err)}`,
